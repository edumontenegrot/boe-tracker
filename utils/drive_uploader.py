...
def _build_service():
    ...
    if not sa_json:
        raise EnvironmentError(
            f"Environment variable {ENV_SA_JSON} is not set."
        )
...
class DriveUploader:
    def __init__(self):
        ...
        if not self.root_folder_id:
            raise EnvironmentError(
                f"Environment variable {ENV_ROOT_FOLDER} is not set."
            )
    ...
    def upload_sumario(
        ...
        return self._upload_bytes(
            folder_id, "sumario.json", content, MIME_JSON
        )
    ...
    def _ensure_bulletin_folder(self, bulletin_id: str, pub_date: str) -> str:
        ...
        year_id = self._get_or_create_folder(year, root)
    ...
    def _ensure_pdf_folder(self, bulletin_id: str, pub_date: str) -> str:
        bulletin_folder_id = self._ensure_bulletin_folder(bulletin_id, pub_date)
        return self._get_or_create_folder("pdfs", bulletin_folder_id)
